...
import {
  Badge, ScreenHeader, EmptyState, PrivacyNote, SafetyMenu
} from '../components/SharedComponents';
import {
  MessageCircle, Shield, ShieldCheck, MapPin, Lock, Send, ChevronRight,
  MoreVertical, Heart, FileText, CheckCircle2, Eye
} from 'lucide-react';
  ...
  return (
    <div data-testid="conversation-thread" className="absolute inset-0 z-30 bg-[#F8F7F4] flex flex-col">
      ...
      <SafetyMenu
        ...
        onReport={() => { alert('Reported'); setShowSafety(false); }}
        onBlock={() => { alert('Blocked'); setShowSafety(false); }}
        onViewContext={() => setShowSafety(false)}
      />
    </div>
  );
}
